...
Last reviewed: 2026-07-01. **KNOWN GAP: the LIVE Play Store listing body copy was never rebranded — it still reads "Brimm" throughout, with dead tagline and dead pricing (checked 2026-07-01). Paste source to fix it: `docs/playstore-listing.md`. The developer display name also shows "pan-tree".** (Header table refreshed 2026-05-12; user-facing brand is now Speakeater. Brimm → Speakeater rebrand completed 2026-04-26 in code — see Migration Log for the swap commit. The 2026-04-26 audit table further down still cites `Brimm` in counts because that audit predates the rebrand; refer to the canonical table below for current state.)
...
| **User-facing brand** | `Speakeater` | App display name (`strings.xml` → `app_name`), marketing copy, Play Store listing, domain (`speakeater.com`), email (`support@speakeater.com`) | **Mutable.** A future rebrand changes ONLY `Brand.kt` + `values/strings.xml` + `social/src/lib/brand.ts` + `landing/src/brand.ts`. The Android `applicationId` (`app.brimm`) is locked because the Play Store listing was created under it before the rebrand. |
...
| Domain | `speakeater.com` | Registered + DNS configured. Rebrand later means new domain + 301 redirects, not a rename. |
| Support email | `support@speakeater.com` | Tied to domain. |
...
object Brand {
  ...
  const val SUPPORT_EMAIL = "support@speakeater.com"
}
```
...
export const BRAND = {
  name: 'Brimm',
  domain: 'speakeater.com',
  supportEmail: 'support@speakeater.com',
  tagline: '...',
};
```
...
If you ever rebrand from "Speakeater" to anything else, this is the entire playbook:
...
- [ ] `pantree-social/src/lib/creator-outreach.ts:6,62` — outreach brief copy → "Speakeater"
- [ ] `pantree-social/src/lib/press-outreach.ts:7-8,67,77` — press brief copy → "Speakeater"
- [ ] `pantree-social/src/lib/recipe-derive.ts:96` — headnote instruction → "Speakeater"
...
- [ ] Create `android/app/src/main/java/app/pantrie/Brand.kt` with `APP_NAME = "Speakeater"` and friends
- [ ] Audit every hardcoded `"Speakeater"` in Kotlin files; replace with `Brand.APP_NAME` or `stringResource(R.string.app_name)`
...
| 2026-04-26 | A | Doc cleanup: README.md launch checklist (`app.pantrie` → `app.brimm`, `pantrie.app` → `speakeater.com`), BETA_RUNBOOK.md (3× pan-tree → Brimm), demo/pantrie-demo.html badge (PANTRIE → BRIMM) | content updated |
...
| 2026-04-26 | D | TS env vars: `PANTREE_BACKEND_URL` → `PANTRIE_BACKEND_URL` (admin-fetch.ts, traffic/page.tsx); `PANTREE_SHARE_URL` → `PANTRIE_SHARE_URL` (post.ts). User must update `.env` to match new var names. Default fallback URL in post.ts changed from `pantrie.app` → `speakeater.com`. | edits applied |
| 2026-04-26 | E | TS legacy text cleanup: anthropic.ts (`PANTREE_BRIEF` → `BRIMM_BRIEF`, removed codename disclaimer, fixed Hacker News title + thread reply rules), creator-outreach.ts (`pan-tree` → `Brimm`, "Free Pro" → "Free Speakeater Pro"), press-outreach.ts (`PANTREE_PRESS_BRIEF` → `BRIMM_PRESS_BRIEF`, sign-off team name), recipe-derive.ts (headnote rule). | edits applied |
...
| 2026-04-26 | rebrand | Brimm → Speakeater swap. Created `Brand.kt`, updated 15 hardcoded "Speakeater" Kotlin strings → `Brand.APP_NAME`/`Brand.PRO_NAME`, updated `strings.xml` `app_name` → "Speakeater". | build verified |
